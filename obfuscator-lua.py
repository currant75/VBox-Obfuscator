...
Auto-detect Download folder | Single-file deployment
...
# ==================== AUTO-DETECT DOWNLOAD FOLDER ====================

def get_download_folder():
    """Auto-detect the user's Downloads folder across platforms"""
...
class LuaObfuscator:
    ...
    def _control_flow_flattening(self, code):
        ...
        for line in lines:
            ...
            obf += f"if {opaque} then {line} {state_var}={rand_int(1,9999)}; else {state_var}={rand_int(1,9999)}; "
    ...
    def _vm_obfuscation(self, code):
        ...
        vm_code = f"""-- VM Protection Layer v4.0 (EXTREME)
...
"""
    ...
    def _bytecode_encryption(self, code):
        ...
        bc_code = f"""-- Bytecode Encryption Layer
...
"""
    ...
    def _dynamic_loader(self, code):
        ...
        dl_code = f"""-- Dynamic Loader Layer
local {mem_name}={{}}
...
"""
    ...
    def _anti_tamper(self, code):
        ...
        at_code = f"""-- Anti-Tamper Protection (EXTREME)
...
"""
    ...
    def _anti_debug(self, code):
        ...
        ad_code = f"""-- Anti-Debug / Anti-Hook Protection (EXTREME)
...
"""
    ...
    def _anti_memory(self, code):
        ...
        mem_code = f"""-- Anti Memory Scan Protection
...
"""
    ...
    def _encrypted_constant_pool(self, code):
        ...
        if constants:
            ...
            cp_code = f"""-- Encrypted Constant Pool
...
"""
    ...
    def _anti_dump(self, code):
        ...
        dump_code = f"""-- Anti-Dump Protection
...
"""
    ...
    def _code_mutation(self, code):
        ...
        mut_code = f"""-- Polymorphic Code Mutation
...
""" + code
    ...
    def _env_fingerprinting(self, code):
        ...
        env_code = f"""-- Environment Fingerprinting
...
"""
    ...
    def _self_healing(self, code):
        ...
        heal_code = f"""-- Self-Healing Mechanism
...
"""
    ...
    def _crash_injection(self, code):
        ...
        trap_code = f"""-- Crash Injection Traps
...
"""
    ...
    def _opaque_predicates(self, code):
        ...
        op_code = f"""-- Opaque Predicate Guards
...
"""
    ...
    def _dead_code(self, code):
        ...
        dead_code = f"""-- Dead Code Insertion
...
"""
    ...
    def _register_virtualization(self, code):
        ...
        reg_code = f"""-- Register Virtualization
local {reg_var}={{}}
...
"""
    ...
    def _dispatch_table(self, code):
        ...
        dt_key = rand_str(16)
        dt_code = f"""-- Dispatch Table Encryption
local {dt_var}={{}}
local _dtKey="{dt_key}"
for i=1,100 do
...
"""
    ...
    def _final_wrapper(self, code):
        ...
        final_code = f"""-- Final Encryption Wrapper
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=f"""
╔══════════════════════════════════════════════════════════════╗
║  CARA PENGGUNAAN (AUTO-DETECT DOWNLOAD FOLDER)               ║
╠══════════════════════════════════════════════════════════════╣
║  Download Folder terdeteksi: {download_folder:<35} ║
╠══════════════════════════════════════════════════════════════╣
║  1 FILE (Basic):                                             ║
║    python3 lua_obfuscator_max.py -i script.lua -o out.lua    ║
║                                                              ║
║  1 FILE (Auto output ke Download):                           ║
║    python3 lua_obfuscator_max.py -i script.lua --max         ║
║    → Output: {download_folder}/script_protected.lua          ║
║                                                              ║
║  BATCH (semua .lua di Download):                             ║
║    python3 lua_obfuscator_max.py -d {download_folder}/ --max ║
║    → Output: {download_folder}/obfuscated/                   ║
║                                                              ║
║  TINGKAT TERAMAN:                                            ║
║    python3 lua_obfuscator_max.py -i in.lua --max -v          ║
║        --watermark "Protected"                               ║
╚══════════════════════════════════════════════════════════════╝
        """
    )
    ...
    group.add_argument('-d', '--directory', help='Folder berisi file .lua (batch)')

    parser.add_argument('-o', '--output', help='File output (single) atau folder output (batch). Default: auto ke Download folder')
    ...
    parser.add_argument('--no-control-flow', action='store_true', help='Matikan control flow')
    ...
    parser.add_argument('--no-anti-memory', action='store_true', help='Matikan anti-memory')
    parser.add_argument('--no-const-pool', action='store_true', help='Matikan constant pool')
    ...
    parser.add_argument('--no-env-finger', action='store_true', help='Matikan env fingerprint')
    parser.add_argument('--no-crash', action='store_true', help='Matikan crash injection')

    args = parser.parse_args()

    # Auto-resolve output paths
    if args.input and not args.output:
        # Single file, auto output to Download folder
        base_name = os.path.basename(args.input)
        name, ext = os.path.splitext(base_name)
        args.output = os.path.join(download_folder, f"{name}{args.suffix}{ext}")
    elif args.directory and not args.output:
        # Batch, auto output to Download/obfuscated/
        args.output = os.path.join(download_folder, "obfuscated")

    print(f"""
╔══════════════════════════════════════════════════════════════╗
║     LUA OBFUSCATOR MAX v4.0 — Ultra Security Edition         ║
║     Auto-Download Folder | Single-File Deployment            ║
╠══════════════════════════════════════════════════════════════╣
║  Download Folder: {download_folder:<43} ║
